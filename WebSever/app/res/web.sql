...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th5 19, 2022 lúc 08:32 PM
...
CREATE TABLE `thanhtra` (
  `id_thanhtra` int(11) NOT NULL,
  `id_coso` int(11) NOT NULL,
  `kq_kiemtra` varchar(50) DEFAULT NULL,
...
INSERT INTO `user` (`id_user`, `name_user`, `password`, `email`, `type`) VALUES
(27, 'string', 'string', 'string', 0),
(28, 'admin', 'admin', 'string', 0),
(38, '121', 'string', 'string', 2),
(40, 'b4', 'string', 'string', 2),
(41, 'b5', 'string', 'string', 2),
(42, 't', 'string', 'string@abc.xyz', 2);
...
ALTER TABLE `mau`
  ...
  ADD KEY `id_thanhtra` (`id_thanhtra`);
...
--
ALTER TABLE `mau`
  ADD CONSTRAINT `mau_ibfk_1` FOREIGN KEY (`id_thanhtra`) REFERENCES `thanhtra` (`id_thanhtra`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Các ràng buộc cho bảng `quanly`
--
ALTER TABLE `quanly`
  ADD CONSTRAINT `quanly_fk1` FOREIGN KEY (`id_huyen`) REFERENCES `huyen` (`id_huyen`),
  ADD CONSTRAINT `quanly_ibfk_2` FOREIGN KEY (`id_user`) REFERENCES `user` (`id_user`) ON DELETE CASCADE ON UPDATE CASCADE;
