...
def register_district(request: request_data.DistrictRegister, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_admin)):
    return manager.register_district(request, db)


@router.delete("/delete_district", status_code=status.HTTP_200_OK)
def delete_district(request: request_data.DistrictRegister, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_admin)):
    return manager.delete_district(request, db)


@router.put("/update_district", status_code=status.HTTP_200_OK)
def update_district(request: request_data.DistrictUpdate, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_admin)):
