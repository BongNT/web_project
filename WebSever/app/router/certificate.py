...
def create_certificates(request: request_data.CertificateCreate, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_user)):
    return certificate.create(request, db, current_user)


@router.delete("/{id}/delete", status_code=status.HTTP_200_OK)
def delete_certificate_by_id(id: int, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_user)):
    return certificate.delete_by_id(id, db, current_user)


@router.put("/update", status_code=status.HTTP_200_OK)
def update_certificate_by_id(request: request_data.CertificateUpdate, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_user)):
