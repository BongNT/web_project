...
def register_facilities(request: request_data.FacilityCreate, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_user)):
    return facility.create(request, db, current_user)


@router.post("/{id}/delete", status_code=status.HTTP_200_OK)
def delete_facility_by_id(id: int, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_user)):
    return facility.delete(id, db, current_user)


@router.put("/update", status_code=status.HTTP_200_OK)
def update_facility_by_id(request: request_data.FacilityUpdate, db: Session = Depends(database.get_db), current_user=Depends(oauth2.get_current_user)):
